$(function(){
	...
	$('.NEhBtn').click(function(){
		if(flag){
			...
			$('.NEhBtn i').css({'width':'34px','height':'34px','margin':'28px 23px','backgroundPosition':'-34px 0px'});
			$('#NEhide').stop(true,true).slideDown(300).css({'display':'block'});
			flag = false;
		}else{
			$(this).css({'background':'#C80E2F','transition':'all 0.5s'});
			$('.NEhBtn i').css({'width':'35px','height':'23px','margin':'33px 23px','backgroundPosition':'0 -5px'});
	...
	function ShowBanner(){		
		...
		if(k==1){
			...
			$('#PrevCont h2').text('《原神》手游');
			$('#PrevCont p').text('全球火热进行中！');
			//next
			$('#NextCont o').css({'backgroundImage':'url(img/4-4.jpg)'});
			$('#NextCont h2').text('《王者荣耀》');
			$('#NextCont p').text('国民MOBA手游大作！');
		}else if(k==2){
			//prev
			$('#PrevCont o').css({'backgroundImage':'url(img/3-3.jpg)'});
			$('#PrevCont h2').text('《碧蓝航线》');
			$('#PrevCont p').text('亲临海战 美少女养成！');
			//next
			...
			$('#PrevCont h2').text('《王者荣耀》');
			$('#PrevCont p').text('国民MOBA手游大作！');
			//next
			$('#NextCont o').css({'backgroundImage':'url(img/5-5.jpg)'});
			$('#NextCont h2').text('《光遇》');
			$('#NextCont p').text('光是遇见 就很美好');
		}else if(k==4){
			//prev
			...
			$('#NextCont h2').text('《原神》手游');
			$('#NextCont p').text('全球火热进行中！');
		}else if(k==0){
			//prev
			$('#PrevCont o').css({'backgroundImage':'url(img/5-5.jpg)'});
			$('#PrevCont h2').text('光遇');
			$('#PrevCont p').text('光是遇见 就很美好');
			//next
			$('#NextCont o').css({'backgroundImage':'url(img/3-3.jpg)'});
			$('#NextCont h2').text('《碧蓝航线》');
			$('#NextCont p').text('亲临海战 美少女养成！');
		}
		clearInterval(timer);
