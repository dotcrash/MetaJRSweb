...
export default {
  ...
  methods: {
	...
	readFile(imgurl){
		...
		var file1 = new File( 'F:/HBuilderX软件/Project/brainHealth/public/static/img/avatar/1.jpg', {type: 'text/plain'});
		    reader.readAsDataURL(file1) // input.files[0]为第一个文件 */
		   /* reader.onload = ()=>{
		      const img = new Image()
		      img.src = reader.result
		      document.body.appendChild(img)  // reader.result为获取结果
		    } */
			/* console.log('F:/HBuilderX软件/Project/brainHealth/public'+imgurl)
			 return require('F:/HBuilderX软件/Project/brainHealth/public'+imgurl);; */
